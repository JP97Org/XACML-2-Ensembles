...
def main(args: Array[String]) : Unit = {
    ...
    val subjectL = new scenario.Subject("L", "Production_Line", "Worker", "Early-Production")
    ...
    val ok = solved && scenario.rootEnsemble.instance.accessFactoryRule.selectedMembers.exists(x => convertToCol(x.allowedSubjects).contains(subjectW) && !convertToCol(x.allowedSubjects).contains(subjectL) && !convertToCol(x.allowedSubjects).contains(subjectF) && !convertToCol(x.allowedSubjects).contains(subjectX)) &&
                       scenario.rootEnsemble.instance.checkTimeToWorkingPlaceRule.selectedMembers.exists(x => !convertToCol(x.allowedSubjects).contains(subjectW) && !convertToCol(x.allowedSubjects).contains(subjectL) && convertToCol(x.allowedSubjects).contains(subjectF) && !convertToCol(x.allowedSubjects).contains(subjectX)) &&
                       ...
                       scenario.rootEnsemble.instance.enterWorkingPlaceRule.selectedMembers.exists(x => !convertToCol(x.allowedSubjects).contains(subjectW) && convertToCol(x.allowedSubjects).contains(subjectL) && !convertToCol(x.allowedSubjects).contains(subjectF) && !convertToCol(x.allowedSubjects).contains(subjectX));
    
    val scenario2 = new RunningExample(LocalTime.parse("14:05:00Z", DateTimeFormatter.ISO_OFFSET_TIME))
    ...
    val ok2 = solved2 && scenario2.rootEnsemble.instance.leaveWorkingPlaceRule.selectedMembers.exists(x => !convertToCol(x.allowedSubjects).contains(subjectW) && convertToCol(x.allowedSubjects).contains(subjectL) && !convertToCol(x.allowedSubjects).contains(subjectF) && !convertToCol(x.allowedSubjects).contains(subjectX)) &&
                         scenario2.rootEnsemble.instance.leaveFactoryRule.selectedMembers.exists(x => convertToCol(x.allowedSubjects).contains(subjectW) && !convertToCol(x.allowedSubjects).contains(subjectL) && !convertToCol(x.allowedSubjects).contains(subjectF) && !convertToCol(x.allowedSubjects).contains(subjectX));                   
    if(ok && ok2) {
      println("OK")
    } else {
      println("NOT OK")
